...
Originally RazorCoin was developed in late 2013 as a rival to BitCoin: Razormind (the company producing it) chose instead to open their Digital Currency Factory for business interested in developing their own Digital Currencies, and related infrastructure. The production of tested and secure wallets, exchanges, web-applications, e-commerce stores became essential for many of the new currencies while being impossible in practice due to inexperienced, or unavailable developers. The original remit was to make the market safer for decentralized digital currencies by guaranteeing RazorCoin currencies and related products for life. 

Innovations include: Gold-backed Cryptocurrencies, Pabble (live embedded DC coin with NFC tech), RefugeeCoin (working with the UN to create Aid packages for victims of war), CafeCoin (for chatting, gameplay, buying drinks, dating), custom-blockchains (for implicit multisig), and Cloud Coins (capable of absorbing features of and being compatible with existing Digital Currencies). 
