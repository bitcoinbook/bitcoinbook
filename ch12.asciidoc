...
In this example, we have five participants: Alice, Bob, Carol, Diana, and Eric. These five participants have opened payment channels with each other, in pairs. Alice has a payment channel with Bob. Bob is connected to Carol, Carol to Diana, and Diana to Eric. For simplicity let's assume each channel is funded with 200,000,000 satoshis (2 bitcoin) by each participant, for a total capacity of 400,000,000 satoshis (4 bitcoin) in each channel. Keep in mind that the intermediate participants (Bob, Carol, Diana), keep separate balances for each channel, and have funded a total of 400,000,000 satoshis (4 bitcoin) among their channels.
...
Alice's node then constructs an HTLC, payable to the hash +H+, with a 10-block refund timeout (current block + 10), for an amount of 100,300,000 satoshis (1.003 bitcoin) (see <<ln_payment_process>> step 2). The extra 300,000 satoshis will be used to compensate the intermediate nodes for their participation in this payment route. Alice offers this HTLC to Bob, deducting 100,300,000 satoshis from her channel balance with Bob and committing it to the HTLC. The HTLC has the following meaning: _"Alice is committing 100,300,000 satoshis of her channel balance to be paid to Bob if Bob knows the secret, or refunded back to Alice's balance if 10 blocks elapse."_ The channel balance between Alice and Bob is now expressed by commitment transactions with three outputs: 200,000,000 satoshis balance to Bob, 99,700,000 satoshis balance to Alice, 100,300,000 satoshis committed in Alice's HTLC. Alice's balance is reduced by the amount committed to the HTLC.

Bob now has a commitment that if he is able to get the secret +R+ within the next 10 blocks, he can claim the 100,300,000 locked by Alice. With this commitment in hand, Bob's node constructs an HTLC on his payment channel with Carol. Bob's HTLC commits 100,200,000 satoshis to hash +H+ for 9 blocks, which Carol can redeem if she has secret +R+ (see <<ln_payment_process>> step 3). Bob knows that if Carol can claim his HTLC, she has to produce +R+. If Bob has +R+ in nine blocks, he can use it to claim Alice's HTLC to him. He also makes 100,000 satoshis for committing his channel balance for nine blocks. If Carol is unable to claim his HTLC and he is unable to claim Alice's HTLC, everything reverts back to the prior channel balances and no one is at a loss. The channel balance between Bob and Carol is now: 200,000,000 to Carol, 99,800,000 to Bob, 100,200,000 committed by Bob to the HTLC.

Carol now has a commitment that if she gets +R+ within the next nine blocks, she can claim 100,200,000 satoshis locked by Bob. Now she can make an HTLC commitment on her channel with Diana. She commits an HTLC of 100,100,000 satoshis to hash +H+, for eight blocks, which Diana can redeem if she has secret +R+ (see <<ln_payment_process>> step 4). From Carol's perspective, if this works she is 100,000 satoshis better off and if it doesn't she loses nothing. Her HTLC to Diana is only viable if +R+ is revealed, at which point she can claim the HTLC from Bob. The channel balance between Carol and Diana is now: 200,000,000 to Diana, 99,900,000 to Carol, 100,100,000 committed by Carol to the HTLC.

Finally, Diana can offer an HTLC to Eric, committing 100,000,000 satoshis for seven blocks to hash +H+ (see <<ln_payment_process>> step 5). The channel balance between Diana and Eric is now: 200,000,000 to Eric, 100,000,000 to Diana, 100,000,000 committed by Diana to the HTLC.

However, at this hop in the route, Eric _has_ secret +R+. He can therefore claim the HTLC offered by Diana. He creates a new commitment transaction using +R+ to claim the 100,000,000 satoshis from Diana's HTLC, adding it to his channel balance (see <<ln_payment_process>> step 6). The channel balance is now: 100,000,000 to Diana, 300,000,000 to Eric.

Now, Diana has secret +R+. Therefore, she can now claim the HTLC from Carol. Diana transmits +R+ to Carol and adds the 100,100,000 satoshis to her channel balance (see <<ln_payment_process>> step 7). Now the channel balance between Carol and Diana is: 99,900,000 to Carol, 300,100,000 to Diana. Diana has "earned" 100,000 for participating in this payment route, as her current balance of 100,000,000 satoshis (channel with Eric) plus 300,100,000 satoshis (channel with Carol) adds to 400,100,000 satoshis.

Flowing back through the route, the secret +R+ allows each participant to claim the outstanding HTLCs. Carol claims 100,200,000 from Bob, setting the balance on their channel to: 99,800,000 to Bob, 300,200,000 to Carol (see <<ln_payment_process>> step 8). Finally, Bob claims the HTLC from Alice (see <<ln_payment_process>> step 9). Their channel balance is updated as: 99,700,000 to Alice, 300,300,000 to Bob.

Alice has paid Eric 100,000,000 satoshis without opening a channel to Eric. None of the intermediate parties in the payment route had to trust each other. For the short-term commitment of their funds in the channel they are able to earn a small fee, with the only risk being a small delay in refund if the channel was closed or the routed payment failed.
