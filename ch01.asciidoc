...
((("bitcoin", "defined", id="GSdefine01")))Bitcoin is a collection of concepts and technologies that form the basis of a digital money ecosystem. Units of currency called bitcoin are used to store and transmit value among participants in the Bitcoin network. 
=== 什麼是比特幣？

((("bitcoin", "defined", id="GSdefine01")))比特幣是構成數字貨幣生態系統基礎的概念和技術的集合。 稱為比特幣的貨幣單位用於在比特幣網絡的參與者之間存儲和傳輸價值。

Bitcoin users communicate with each other using the Bitcoin protocol primarily via the internet, although other transport networks can also be used. The Bitcoin protocol stack, available as open source software, can be run on a wide range of computing devices, including laptops and smartphones, making the technology easily accessible.
比特幣用戶主要通過互聯網使用比特幣協議相互通信，但也可以使用其他傳輸網絡。 比特幣協議棧作為開源軟件提供，可以在各種計算設備上運行，包括筆記本電腦和智能手機，使該技術易於訪問。
...
用戶可以通過網絡傳輸比特幣來做任何可以用傳統貨幣做的事情，包括買賣商品、向個人或組織匯款，或者提供信貸。 比特幣可以在專門的貨幣交易所購買、出售和兌換成其他貨幣。 從某種意義上說，比特幣是互聯網貨幣的完美形式，因為它快速、安全且無國界。
