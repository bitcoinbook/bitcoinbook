...
Bitcoin clients typically fall into one of these categories:

Full Client:: A full client, or "full node" is a client that stores the entire history of bitcoin transactions, manages the user's wallet(s) and can initiate transactions directly on the bitcoin network. This is similar to a standalone email server, in that it handles all aspects of the protocol without relying on any other servers or third party services.
...
Hybrid Client:: Hybrid clients mix and match attributes from the other categories in order to achieve a balance of concerns between control, ease of use and security. For example, a hybrid client may provide password protected encrypted-only storage of a user's wallet, or require M-of-N keys to sign any transaction spending from the user's wallet.

.Mobile Bitcoin
****
Mobile clients for smartphones, such as those based on the Android system, can operate as any of the categories of clients listed above. Some mobile clients are synchronized with a web or desktop client, providing a multi-platform wallet across multiple devices but with a common source of funds. See <<mobile_bitcoin>>
****

The choice of bitcoin client depends on how much control the user wants over funds. A full client can offer a high level of control and independence for the user, but in turn puts the burden of backups and security on the user. On the other end of the range of choices, a web client is the easiest to set up and use, but the tradeoff with a web client is that counterparty risk is introduced because security and control is shared by the user and the owner of the web service. If a web-wallet service is compromised, as many have been, the users can lose all their funds. Conversely, if a user has a full client without adequate backups, they may lose their funds through a computer mishap. 
...
A non-full client is the quickest way to start using bitcoin, and is the choice of Alice who we introduced in <<user-stories>>. Alice is not a technical user and only recently heard about bitcoin from a friend. She starts her journey by visiting the official website bitcoin.org, where she finds a broad selection of bitcoin clients. Following the advice on the bitcoin.org site, she chooses a wallet by blockchain.info, a popular hosted-wallet service. Following a link from bitcoin.org, she opens the blockchain.info wallet page at https://blockchain.info/wallet and selects "Start a New Wallet". To register her new wallet, all she must  do is enter a password.
...
A few seconds later, Alice can start using her new bitcoin wallet by logging in with her account ID and password. In her web browser, she sees the wallet home screen:
...
It is not possible to buy bitcoins at a bank or foreign exchange kiosk at this time. It is not possible to use a credit card to buy bitcoins, either. As of 2014, it is still quite difficult to acquire bitcoins in most countries. There are a number of specialized currency exchanges where you can buy and sell bitcoin in exchange for a local currency. These operate as web-based currency exchanges and include:
...
* Coinbase (coinbase.com), a US-based company in California that supports US dollar exchange to and from bitcoin. Coinbase can connect to US checking accounts via the ACH system.
...
* Find a bitcoin "faucet" that will give you a small amount of bitcoin, usually after doing something simple like entering a CAPTCHA.

Alice was introduced to bitcoin by a friend and so she has an easy way of getting her first bitcoin while she waits for her account on an exchange to be verified and activated. 
