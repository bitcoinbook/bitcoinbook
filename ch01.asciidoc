...
Unlike traditional currencies, bitcoins are entirely virtual. There are no physical coins or even digital coins per se. The coins are implied in transactions which transfer a quantitative value from sender to recipient. Users of bitcoin own keys which allow them to prove ownership of addresses in the bitcoin network, allowing them to unlock the value associated with them and transfer the value to a new recipient. Those keys are often stored in a digital wallet on the user’s computer. Possession of a key associated with an address is the only prerequisite to spending bitcoins transferred to it, putting the control entirely in the hands of the user.
