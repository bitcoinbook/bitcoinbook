...
When Alice runs Bluewallet for the first time, she chooses the option to create a new wallet, and takes a moment **away from Joe and all other parties** to write down a secret mneumonic phrase _in order_ on a piece of paper. As explained the by mobile wallet and by Joe earlier, the mneumonic phrase allows Alice to restore her wallet in case she loses her mobile device and grants her access to her funds on another device. After creating her wallet, Alice can tap it to see her bitcoin amount, transaction history, as well as two buttons that allow her to either _recieve_ or _send_ bitcoin, shown in <<bluewallet-welcome>>.
