...
Bitcoin is collection of concepts and technologies that form the basis of a digital money ecosystem.  

JB - just wondered if the opening could be a bit less conceptual eg 'Bitcoin is a system of digital money'? The definition can then be expanded upon later as the reader's understanding grows. Maybe it is me only, but I find it harder to grasp something new when it is described conceptually at the start....
 
It includes a currency, with units called bitcoins, that are used to store and transmit value among participants in the bitcoin network.  

JB - are bitcoins used to store value at the moment, or just transfer value? As a beginner it seems to me that bitcoins aspire to be a predictable store of value but are not at the moment as it is all quite volatile...  
...
JB - interesting point: what other transport networks?
...
JB - how would credit be extended? 

Bitcoin technology includes features, based on encryption and digital signatures, to ensure the security of the bitcoin network. 

JB - suggest the technology more than 'includes' encryption, it has encryption at its very core.

Bitcoins can be purchased and sold, exchanged for other currencies at a floating exchange rate, at specialized currency exchanges. 

JB - sentence reads a bit odd?

Bitcoin in a sense is the perfect form of money for the Internet: fast, secure, borderless. 

JB - suggest a general perception of bitcoin is that it is not secure (eg liable to loss and theft). Maybe caveat that it is very secure as a protocol etc, but that users must take certain steps to make it secure for them to actually use? 
...
JB - quite a lot of new concepts in this one para... Also, does one ever 'spend' the coins? Isn't it just that ownership is transferred (kind of in the way that ownership of land is not spent it is transferred)? With ref to the last sentence, isn't control only entirely in the hands of the user whilst the bitcoin network remains, and remains secure?? And another pre-req is access to the network? My private key cannot be used to transfer value without access to the network as I understand it.
...
JB - is this in theory only now? I understand it would be costly of electricity for an individual to attempt bitcoin mining. Perhaps caveat that nowadays mining is very specialised and carried out mostly by professional miners / at dedicated data centres?
...
JB - there seems to me a conflict of aims here, as competition surely tends to centralise, the goal of competition being to win not to share? 
...
JB - I am no computer expert but do any blocks get CPU mined these days?

The protocol also halves the rate at which new bitcoins are created every 4 years, and limits the total number of bitcoins that will be created to a fixed total of 21 million coins. The result is that the number of bitcoins in circulation closely follows an easily predictable curve that  reaches 21 million by the year 2140.  As a result, the bitcoin currency is deflationary and cannot be inflated by "printing" new money above and beyond the expected issuance rate. 

JB - is it good that BTC is limited to 21m? Presume this is up for much theoretical debate and that there are advantages and disadvantages to both sides?
...
JB - historically, were other digital money attempts made using something other than cryptography? Would be an interesting point of note.
...
JB - give couple of examples of these initial digital currencies? (I think you may have done this later on, but they are first mentioned here...)
...
JB - as above, does not the design tend away from 'complete decentralisation' because miners are competing to be the one and only winner?? 
...
The bitcoin network started in 2009, based on a reference implementation published by Nakamoto and since revised by many other programmers. During the first four years of operation, the network has grown to include an enormous amount of Proof-Of-Work computation, thereby increasing its security and resilience. 

In 2013, the total market value of bitcoin's primary monetary supply measure (M0) is estimated at more than 10 billion US dollars. The largest transaction processed by the network was $150 million US dollars, transmitted instantly and processed without any fees.
...
JB - how does the protocol get developed? Isn't there a core development team or something like that? If yes I assume they are exerting some control, or trying to?
...
JB - I felt that this section broke up the narrative a bit. I wondered if you could just jump into these stories individually as and when you actually use them?
...
JB - at what point are you recommending she is 'now' the owner - ie after how many confirmations can she be confident that the sale with Joe is concluded and they can safely take their leave?
