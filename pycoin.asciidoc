...
Yep, if the file passed to `-f` ends with _.gpg_, then `gpg -d` is automatically invoked, and you can type your GnuPG passphrase. So, `tx` plus `gpg` is actually a pretty reasonably secure solution! If you keep your _WIF.gpg_ file on an airgapped machine this, additionally, solves the problem of cold storage!
