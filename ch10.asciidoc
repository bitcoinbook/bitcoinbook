...
A higher block size is in the interest of the miners since more transactions incur more fees. But joining the networks is more expensive since more powerful hardware is required to process the larger blocks. This puts the system at risk of more centralization. Less intrusive techniques were known to include more transactions in a block, most notably segwit. There was a deep divide in the community on whether to implement the block size increase, and a split occured, resulting in the birth of an alternate chain with a higher block size limit, Bitcoin Cash (BCH). A minority of Bitcoin miners switched to this new chain.
===
