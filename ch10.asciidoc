...
Examples of software forks that have attempted to change consensus rules include Bitcoin XT, Bitcoin Classic and Bitcoin Unlimited. However, none of these software forks have resulted in a hard fork. While a software fork is a necessary precondition, it is not in itself sufficient. For a hard fork to occur, the competing implementation must be adopted and the new rules activated, by miners, wallets, and intermediary nodes. Conversely, there are numerous alternative implementations of Bitcoin Core, and even software forks, that do not change the consensus rules and barring a bug, can coexist on the network and interoperate without causing a hard fork.
...
[NOTE]
===
In 2017, an especially contentious hard fork was proposed to change the consensus rules controlling the maximum block size limit in order to make space for extra transactions. The intent was to address the high transaction fees. Due to the increased usage of the Bitcoin network, users had to compete on fees to get their transactions included.

A higher block size is in the interest of the miners since more transactions incur more fees. But joining the network is more expensive since more powerful hardware is required to process the larger blocks. This puts the system at risk of more centralization. Less intrusive techniques were known to include more transactions in a block, most notably segwit. There was a deep divide in the community on whether to implement the block size increase, and a split occured, resulting in the birth of an alternate chain with a higher block size limit, Bitcoin Cash (BCH). A minority of Bitcoin miners switched to this new chain.
===
