...
By being included in a block, Alice's transaction becomes "confirmed". That is, block #277316 becomes the first confirmation for Alice's transaction. A few minutes later, a new block, #277317 is mined by another miner. As this new block is based on the previous block (#277316) that contained Alice's transaction, it added even more computation on top of that block, thereby strengthening the trust in those transactions. This new block acts as a second confirmation of Alice's transaction. As the blocks pile on top of each other, it becomes exponentially harder to reverse the transaction, thereby making it more and more trusted by the network. 
