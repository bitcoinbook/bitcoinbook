...
((("libraries, alternative","bitcore")))((("bitcore")))((("JavaScript,"bitcore")))http://bitcore.io[_bitcore_] is an open source library sponsored by ((("BitPay"))) BitPay, aiming at providing a simple and powerful API for both browser and Node.js applications. It features private key management, building transactions, verifying scripts, and deriving BIP 32 extended keys. There is a set of external modules that extend its features, adding Payment Protocol (BIP 70) support, access to the P2P network, and compatibility with various BIPs.

===== bitcore Playground

There's a website called _bitcore Playground_ (http://bitcore.io/playground[http://bitcore.io/playground]) that provides a GUI for some common tasks when working with bitcoin: creating a private key, deriving a master extended key, or crafting a transaction. With this website, new users can explore the API without seting up an environment.

===== Installing bitcore

_bitcore_ is available through both https://npmjs.com[_npm_] and http://bower.io[_bower_] package management systems. To install it, simply use
...
The API was designed to be as easy to use as possible. Take a look at this code that builds the first non-coinbase transaction by Satoshi on block 170.

====
[source,javascript]
----
include::code/bitcore_example.js[]
----
====
