...
There is a mathematical relationship between the public and the private key that allows the private key to be used to generate signatures on messages. These signatures can then be checked against the public key to verify ownership, without the bitcoin owner revealing their private key at any stage during the process.
...
Additionally, the BIP0038 encryption scheme takes a passphrase -- a long password -- usually composed of several words or a complex string of alphanumeric characters. The result of the BIP0038 encryption scheme is a Base58Check encoded encrypted private key that begins with the prefix +6P+. If you see a key that starts with +6P+ that means it is encrypted and requires a passphrase in order to convert (decrypt) it back into a WIF-formatted private key (prefix +5+) that can be used in any wallet. Many wallet applications now recognize BIP0038 encrypted private keys and will prompt the user for a passphrase to decrypt and import the key. Third party applications, such as the incredibly useful browser-based https://bitaddress.org (Wallet Details tab), can be used to decrypt BIP0038 keys. 
...
Test the following encrypted keys using https://bitaddress.org to see how you can get the decrypted key by entering the passphrase:
...
Another way to find a vanity address is to outsource the work to a pool of vanity-miners, such as the pool at https://vanitypool.appspot.com. A pool is a service that allows those with GPU hardware to earn bitcoin searching for vanity addresses for others. For a small payment (0.01 bitcoin or approximately $5 when this was written), Eugenio can outsource the search for a 7-character pattern vanity address and get results in a few hours instead of having to run a CPU search for months. 
...
.An example of an encrypted paper wallet from https://bitaddress.org. The passphrase is "test"
...
.An example of a paper wallet from https://bitcoinpaperwallet.com with the private key on a folding flap.
image::images/paper_wallet_bpw.png["paper_wallet_bpw"]

.The https://bitcoinpaperwallet.com paper wallet with the private key concealed.
